                         ...
                         uint8_t len, uint8_t reg, uint8_t *data)
{
  if ((id >= TWI_ID_SIZE)
      || (len == 0)
      || (len > MAX_LEN)
      || (buf_space (&queue) < len + OVERHEAD))
    return 0;

  if ((buf_byte_fill (&queue, (len << 1) | ACTION_WRITE) == 0)
      || (buf_byte_fill (&queue, id) == 0)
      || (buf_byte_fill (&queue, tag) == 0)
      || (buf_byte_fill (&queue, reg) == 0))
    return 0;                   /* ? */
  ...
  for (uint8_t i = 0; i < len; ++i)
    if (buf_byte_fill (&queue, data[i]) == 0)
      return 0;                 /* ? */
...
uint8_t twi_read_array (uint8_t id, uint8_t tag, uint8_t len, uint8_t reg)
{
  if ((id >= TWI_ID_SIZE)
      || (len == 0)
      || (len > MAX_LEN)
      || (buf_space (&queue) < OVERHEAD))
    return 0;

  if ((buf_byte_fill (&queue, (len << 1) | ACTION_READ) == 0)
      || (buf_byte_fill (&queue, id) == 0)
      || (buf_byte_fill (&queue, tag) == 0)
      || (buf_byte_fill (&queue, reg) == 0))
    return 0;
