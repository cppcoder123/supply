...
static void render ()
{
  uint8_t tmp = rtc (buffer[BUFFER_HOUR], RTC_FROM, RTC_HOUR);
  if (tmp != hour)
    gui_update (PARAM_WATCH_HOUR, tmp);
  hour = tmp;

  tmp = rtc (buffer[BUFFER_MINUTE], RTC_FROM, RTC_MINUTE);
  if (tmp != minute)
    gui_update (PARAM_WATCH_MINUTE, tmp);
  minute = tmp;
}
